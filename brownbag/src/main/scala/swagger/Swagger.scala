...
case class Swagger(system: ActorSystem) extends SwaggerHttpService {
  ...
  override def apiClasses: Set[Class[_]] = Set(classOf[APIDefinition],
    classOf[LogGeneratorTrait],
    classOf[HostnameTrait],
    classOf[CountWordsTrait])
  ...
  override def info: Info =
    new Info(
      "Swagger Akka HTTP Brownbag Demo",
      "1.0",
      "Swagger API",
      "",
      None,
